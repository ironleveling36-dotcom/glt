...
# ─────────────────────────────────────────────────────────────────────────────
# GTL API Client  (unchanged from original)
# ─────────────────────────────────────────────────────────────────────────────
...
class GTLClient:
    ...
    def login(self):
        ...
        r    = self.session.post(f"{BASE_URL}/login", json=payload, timeout=15)
        data = r.json()
        otp_token = (
            data.get("token")
    ...
    def verify_otp(self, otp: str):
        r    = self.session.post(f"{BASE_URL}/otp/verify", json={"otp": otp},
    ...
    def signup(self):
        ...
        r    = self.session.post(f"{BASE_URL}/signup", json=payload,
                                 headers=self._auth_headers(), timeout=15)
        data = r.json()
    ...
    def ping(self):
        r = self.session.post(f"{BASE_URL}/ping", json={},
                              headers=self._auth_headers(), timeout=15)
        return r.json()

    def game_data(self, lat=28.6139, lng=77.2090):
        r = self.session.post(f"{BASE_URL}/data", json={"lat": lat, "lng": lng},
                              headers=self._auth_headers(), timeout=15)
        return r.json()

    def game_data_spoof(self):
        best = None
        for lat, lng in LOCATIONS:
            try:
                data = self.game_data(lat, lng)
                if data.get("isInStore") and data.get("store"):
                    store = data["store"]
                    try:
                        elat = float(store.get("lat", lat))
                        elng = float(store.get("lng", lng))
                        ed   = self.game_data(elat, elng)
                        if ed.get("isInStore"):
                ...
                pass
        return best or self.game_data()

    def submit_score(self, game_data=None):
        ...
        def _post(payload):
            ...
            r = self.session.post(f"{BASE_URL}/score", json=payload,
                                  headers=headers, timeout=15)
            return r, r.json()

        try:
            r, resp = _post({"score": 90, "source": "swiggy"})
        ...
        _, resp = _post({"scores": [{"object_id": 1, "score": 10}, {"object_id": 2, "score": 30}]})
        return resp

    def leaderboard(self):
        r = self.session.post(LB_URL, json={"data_type": "match_tickets"}, timeout=15)
        return r.json()
...
def extract_coupon(resp: dict) -> str | None:
    raw = json.dumps(resp)
...
def send_sms_coupon(mobile: str, coupon: str) -> bool:
    try:
        r = requests.get(
...
async def get_otp(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        verify_resp = client.verify_otp(otp)
    except Exception as e:
    ...
    if is_new and not client.token:
    ...
    game_resp = None
    try:
        game_resp = client.game_data_spoof()
    except Exception:
        pass

    await update.message.reply_text("🎯 Score submit ho raha hai...")
    try:
        score_resp = client.submit_score(game_data=game_resp)
    except Exception as e:
    ...
    if coupon:
        ...
        sms_ok = send_sms_coupon(mobile, coupon)
        if sms_ok:
